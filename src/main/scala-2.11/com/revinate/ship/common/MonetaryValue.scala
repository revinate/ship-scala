...
case class MonetaryValue(value: Option[BigDecimal] = None, currency: Option[String] = None) {
  ...
  def +(that: MonetaryValue): Option[MonetaryValue] = {
    lazy val sum: Option[BigDecimal] = (this.value, that.value) match {
    ...
    (this.currency, that.currency) match {
      case (Some(curr), None) => {
  ...
  def *(multiplicand: BigDecimal): MonetaryValue =
    MonetaryValue(value.map(_ * multiplicand), currency)
}
